...
func AlwaysErrorAction[K, A any](err error) ActionFunc[K, A] {
	return func(ctx context.Context, _ K, _ A) error { return err }
}
...
type Transition[K, A any] interface {
	...
	RequeueOnError(k K, a A) ctrl.Result
}
...
type AbstractTransition[K, A any] struct {
	...
	requeueOnError RequeueFunc[K, A]
}
...
func (t *AbstractTransition[K, A]) RequeueOnError(k K, a A) ctrl.Result {
	return t.requeueOnError(k, a)
}
...
func (s *TransitionSet[K, A]) Run(ctx context.Context, k K, a A) (ctrl.Result, error) {
	...
	if err := s.DefaultAction(ctx, k, a); err != nil {
		return s.requeueOnError(k, a), err
	}

	return s.requeue(k, a), nil
}
