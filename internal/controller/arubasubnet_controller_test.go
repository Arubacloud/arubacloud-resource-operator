...
var _ = Describe("ArubaSubnet Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind ArubaSubnet")
			err := k8sClient.Get(ctx, typeNamespacedName, arubaSubnet)
			if err != nil && errors.IsNotFound(err) {
				resource := &v1alpha1.ArubaSubnet{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
					Spec: v1alpha1.ArubaSubnetSpec{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ArubaSubnet")
		...
		It("should successfully reconcile the resource", func() {
			...
			controllerReconciler := &ArubaSubnetReconciler{
