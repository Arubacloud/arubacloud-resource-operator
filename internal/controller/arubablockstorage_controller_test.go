...
var _ = Describe("ArubaBlockStorage Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind ArubaBlockStorage")
			err := k8sClient.Get(ctx, typeNamespacedName, arubaBlockStorage)
			if err != nil && errors.IsNotFound(err) {
				resource := &v1alpha1.ArubaBlockStorage{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
					Spec: v1alpha1.ArubaBlockStorageSpec{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ArubaBlockStorage")
		...
		It("should successfully reconcile the resource", func() {
			...
			controllerReconciler := &ArubaBlockStorageReconciler{
...
var _ = Describe("ArubaBlockStorage Controller Reconcile Method", func() {
	Context("When testing reconcile phases", func() {
		var (
			ctx                context.Context
			reconciler         *ArubaBlockStorageReconciler
			arubaBlockStorage  *v1alpha1.ArubaBlockStorage
			typeNamespacedName types.NamespacedName
		)
		...
		It("should initialize phase when empty", func() {
			...
			arubaBlockStorage = &v1alpha1.ArubaBlockStorage{
				...
				Status: v1alpha1.ArubaBlockStorageStatus{
					ArubaResourceStatus: v1alpha1.ArubaResourceStatus{Phase: ""},
					ProjectID:           "",
				},
			}
		...
		It("should trigger delete phase when DeletionTimestamp is set", func() {
			...
			arubaBlockStorage = &v1alpha1.ArubaBlockStorage{
				...
				Status: v1alpha1.ArubaBlockStorageStatus{
					ArubaResourceStatus: v1alpha1.ArubaResourceStatus{Phase: v1alpha1.ArubaResourcePhaseCreated},
				},
			}
			Expect(k8sClient.Create(ctx, arubaBlockStorage)).To(Succeed())
			...
			updatedBlockStorage := &v1alpha1.ArubaBlockStorage{}
			err = k8sClient.Get(ctx, types.NamespacedName{
				Name:      testName,
				Namespace: "default",
			}, updatedBlockStorage)
			if err == nil {
				Expect(updatedBlockStorage.Status.Phase).To(Equal(v1alpha1.ArubaResourcePhaseDeleting))
			}
		})
		...
		It("should test Next method", func() {
			...
			reconciler.arubaObj = arubaBlockStorage
		...
		It("should test getProjectID method with valid project reference", func() {
			...
			existingProject := &v1alpha1.ArubaProject{}
