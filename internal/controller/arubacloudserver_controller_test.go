...
var _ = Describe("ArubaCloudServer Controller", func() {
	Context("When reconciling a resource", func() {
		...
		arubaCloudServer := &v1alpha1.ArubaCloudServer{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind ArubaCloudServer")
			err := k8sClient.Get(ctx, typeNamespacedName, arubaCloudServer)
			if err != nil && errors.IsNotFound(err) {
				resource := &v1alpha1.ArubaCloudServer{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
					Spec: v1alpha1.ArubaCloudServerSpec{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ArubaCloudServer")
		...
		It("should successfully reconcile the resource", func() {
			...
			controllerReconciler := &ArubaCloudServerReconciler{
...
var _ = Describe("ArubaCloudServer Controller Reconcile Method", func() {
	Context("When testing reconcile phases", func() {
		var (
			ctx                context.Context
			reconciler         *ArubaCloudServerReconciler
			arubaCloudServer   *v1alpha1.ArubaCloudServer
			typeNamespacedName types.NamespacedName
		)
		...
		It("should initialize phase when empty", func() {
			...
			arubaCloudServer = &v1alpha1.ArubaCloudServer{
				...
				Status: v1alpha1.ArubaCloudServerStatus{
		...
		It("should trigger delete phase when DeletionTimestamp is set", func() {
			...
			updatedCloudServer := &v1alpha1.ArubaCloudServer{}
			err = k8sClient.Get(ctx, types.NamespacedName{
				Name:      testName,
				Namespace: "default",
			}, updatedCloudServer)
			if err == nil {
				Expect(updatedCloudServer.Status.Phase).To(Equal(v1alpha1.ArubaResourcePhaseDeleting))
			}
		})
		...
		It("should test Next method", func() {
			...
			reconciler.arubaObj = arubaCloudServer
