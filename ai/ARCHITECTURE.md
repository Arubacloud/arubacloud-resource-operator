...
5. **Aruba client creation** — calls `r.ArubaClient(kubeObj.Spec.Tenant)` to obtain a tenant-scoped `client.Client` (the operator's SDK port, see § "SDK access layer").
...
The operator does **not** call the [Aruba Go SDK](https://github.com/Arubacloud/sdk-go) directly. All CMP access goes through a thin port in `internal/client` (`client.Client` and its per-domain / per-resource sub-clients).

The SDK's public API is object-oriented: resource **wrappers** (`aruba.VPC`, `aruba.Project`, …) built with fluent setters and hydrated only by real client calls (`Get`/`Create`/`Update`/`List`). The operator's reconciliation engine, by contrast, is generic over the SDK's **wire response types** (`*arubatypes.VPCResponse`, etc.) so responses can be constructed directly in unit tests. The port bridges the two: it presents a request-in / wire-response-out surface (`List/Create/Update/Delete` returning `*arubatypes.Response[T]`) and, in `internal/client/adapter.go`, translates to and from the SDK wrappers — reads via `wrapper.Raw()`, writes by building a wrapper from the wire request via setters. Controllers and the transition engine depend only on the port; unit tests mock it. See `REPO.md` § `internal/client/`.
...
The `Reconciler` holds a private per-tenant cache of port clients (`clients map[string]client.Client`, guarded by `clientsMu`), initialized empty in `NewReconciler()`.

When `HandleReconcile` calls `r.ArubaClient(tenant)`:
1. The cache is checked for an existing `client.Client` for the tenant (under `clientsMu`).
2. On a cache miss, a raw `aruba.Client` is built from `ReconcilerConfig`:
...
3. The raw SDK client is wrapped in the port via `client.New(sdkClient)`, cached, and the port client is returned.
...
- Test helpers and fixtures are in `common_test.go` in each package; builder functions follow the pattern `build<Resource>Response(...)` and `default<Resource>Spec(...)`; the `controller` package's `common_test.go` provides `newTestReconciler` and `findCondition`
- Controller tests inject a mock port client (`client.Client`) via `reconciler.NewReconcilerForTest` + a pre-seeded `map[string]client.Client` keyed by tenant (see `CONVENTIONS.md` for the pattern)
