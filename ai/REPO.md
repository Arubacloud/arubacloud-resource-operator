...
- `owner_reference.go` — shared helpers for OwnerReference management: `resolveOwnerObject`, `ensureOwnerReference`, `hasOwnedChildren`, `deleteOwnedChildren`. Used by all child controllers in `HandleReconcile` (to set OwnerReferences) and by parent controllers in the `WaitingChildrenDeletion` transition (`hasOwnedChildren` for the condition check, `deleteOwnedChildren` for the kAction that explicitly triggers child deletion).
