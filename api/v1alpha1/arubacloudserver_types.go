...
// ArubaCloudServerSpec defines the desired state of ArubaCloudServer.
type ArubaCloudServerSpec struct {
	...
	// ProjectReference references the ArubaProject that owns this cloud server
	// +kubebuilder:validation:Required
	ProjectReference ResourceReference `json:"projectReference"`
}

// ArubaCloudServerStatus defines the observed state of ArubaCloudServer.
type ArubaCloudServerStatus struct {
...
// +kubebuilder:resource:scope=Namespaced,shortName=acs
...
// ArubaCloudServer is the Schema for the arubacloudservers API.
type ArubaCloudServer struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   ArubaCloudServerSpec   `json:"spec,omitempty"`
	Status ArubaCloudServerStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// ArubaCloudServerList contains a list of ArubaCloudServer.
type ArubaCloudServerList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []ArubaCloudServer `json:"items"`
}

func init() {
	SchemeBuilder.Register(&ArubaCloudServer{}, &ArubaCloudServerList{})
}
