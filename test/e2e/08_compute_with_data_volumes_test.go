...
var _ = Describe("08-ComputeWithDataVolumes", Ordered, func() {
	const (
		name           = "aruba-test-compute-vol"
	...
	It("should create all resources and CloudServer with data volumes", func(ctx SpecContext) {
		...
		bootVolumeManifest, err := utils.LoadSampleManifest("arubacloud.com_v1alpha1_blockstorage_bootable.yaml", map[string]string{
			"__NAME__":              name,
			"__NAMESPACE__":         namespace,
		...
		cloudServerManifest, err := utils.LoadSampleManifest("arubacloud.com_v1alpha1_cloudserver-with-data-volumes.yaml", map[string]string{
			"__NAME__":                  name,
			"__NAMESPACE__":             namespace,
