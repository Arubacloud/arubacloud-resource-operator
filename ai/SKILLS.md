...
4. Produce a filled-in plan by substituting every `<…>` placeholder with resource-specific content derived from the code and the CMP API. Include the documentation section (§7) — read `ai/DOCS.md` for conventions if needed. Do not include the template instructions or generic guidance in the output — only the completed plan.
