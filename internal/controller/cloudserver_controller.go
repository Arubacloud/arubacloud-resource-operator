...
func (r *CloudServerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	specOrStatusChanged := predicate.Funcs{
		...
		CreateFunc: func(e event.CreateEvent) bool {
			newObj := e.Object.(*v1alpha1.CloudServer)
			log.Println("Create event received for CloudServer", "name", newObj.Name, "namespace", newObj.Namespace)
			return true
		},
		DeleteFunc: func(e event.DeleteEvent) bool {
			newObj := e.Object.(*v1alpha1.CloudServer)
			log.Println("Delete event received for CloudServer", "name", newObj.Name, "namespace", newObj.Namespace)
			return true
		},
	}
