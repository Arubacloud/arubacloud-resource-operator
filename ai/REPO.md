...
- `cmp_error.go` — CMP error types: `CMPError` struct, `CMPErrorCategory` enum (Semantic/Transient/Technical), and `CMPErrorFromResult(operation, resource, err, okStatusCodes...)` — the single categorization entry point that turns the plain `error` returned by a high-level SDK call (Create/Update/Delete) into a `*CMPError`. It classifies `*aruba.HTTPError` replies by status code (4xx with field-level errors → Semantic, 4xx without → Transient, else Technical) and treats `okStatusCodes` (e.g. 404 on delete) as success. `CMPErrorIsSemantic`/`Transient`/`Technical` inspect the chain. Message hygiene lives in `sanitizeCMPString` (collapses tabs/newlines/space runs) and `appendValidationDetail`, which renders field-level validation entries as `"field: message"` pairs and merges them into `Detail`; it takes a local `cmpValidationError` slice rather than the SDK's `pkg/types` values so it stays unit-testable — see `ai/CONVENTIONS.md` § imports.
- `common.go` — `IsFinalState(aruba.State)` and `IsTransitoryState(aruba.State)`, thin classifiers over the SDK's `aruba.State.IsTransitory()` used to decide whether a CMP resource is settled enough to act on. No raw CMP-state string constants remain — the operator compares against `aruba.State*` constants directly.
...
- `NewReconcilerForTest(k8sClient, scheme, mtClient)` — test-only constructor that bypasses `ctrl.Manager` and real credentials; accepts a pre-seeded `arubamt.Multitenant`. Tests seed it with a **real** `aruba.Client` pointed at an `httptest` server (see `internal/controller/common_test.go`'s `fakeCMP`), because the SDK v1.0.4 wrapper objects cannot be populated with server-assigned fields (ID/State/…) outside the `aruba` package, so they cannot be mock-returned.
